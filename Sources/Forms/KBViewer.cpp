			...
			for (m = 0; m < lbKB->SelCount; m++) {
				...
				for (; k2 < lbIDR->Items->Count; k2++) {
					if (lbIDR->Selected[k2]) {
						idrLine = lbIDR->Items->Strings[k2];
						k2++;
						break;
					}
				}
				if (kbLine != "" && idrLine != "") {
					...
					if (pos1 && pos2) {
						...
						pos = idrName.Pos(";");
						if (pos)
							idrName = idrName.SubString(1, pos - 1);
						adr = 0;
						if (TryStrToInt(String("$") + idrName, val))
							adr = val;
						ap = Adr2Pos(adr);
						if (kbName != "" && ap >= 0) {
							recN = GetInfoRec(adr);
