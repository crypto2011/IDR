...
void __fastcall TFExplorer_11011981::ShowCode(DWORD fromAdr, int maxBytes) {
	...
	for (int n = 0; n < maxBytes;) {
		...
		if (n + instrLen > maxBytes)
			break;
		if (pos + instrLen >= CodeSize)
			break;
		pos += instrLen;
		curAdr += instrLen;
		n += instrLen;
...
void __fastcall TFExplorer_11011981::ShowData(DWORD fromAdr, int maxBytes) {
	...
	case 1:
		singleVal = 0;
		memmove((void*)&singleVal, Image + pos, 4);
	...
	case 2:
		doubleVal = 0;
		memmove((void*)&doubleVal, Image + pos, 8);
	...
	case 3:
		try {
			extendedVal = 0;
			memmove((void*)&extendedVal, Image + pos, 10);
	...
	case 4:
		realVal = 0;
		memmove((void*)&realVal, Image + pos, 4);
	...
	case 5:
		compVal = 0;
		memmove((void*)&compVal, Image + pos, 8);
