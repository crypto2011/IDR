...
// ---------------------------------------------------------------------------
void __fastcall TFEditFunctionDlg_11011981::bCancelTypeClick(TObject *Sender) {
	if (!TypModified) {
		PInfoRec recN = GetInfoRec(Adr);
		recN->SetName(SName);
...
void __fastcall TFEditFunctionDlg_11011981::FillType() {
	...
	if (!VmtCandidatesNum) {
		...
		if (VmtCandidatesNum == 1)
			cbVmtCandidates->Text = cbVmtCandidates->Items->Strings[0];

		if (recN->kind == ikConstructor || recN->kind == ikDestructor || (flags & PF_METHOD)) {
			cbMethod->Checked = true;
			if (recN->HasName())
				cbVmtCandidates->Text = ExtractClassName(recN->GetName());
		}
		else {
			cbMethod->Checked = false;
		}
	...
	if (argsBytes > recN->procInfo->retBytes)
		recN->procInfo->flags |= PF_ARGSIZEG;
	if (argsBytes < recN->procInfo->retBytes)
		recN->procInfo->flags |= PF_ARGSIZEL;
...
// ---------------------------------------------------------------------------
void __fastcall TFEditFunctionDlg_11011981::FormClose(TObject *Sender, TCloseAction &Action) {
	if (!TypModified) {
		PInfoRec recN = GetInfoRec(Adr);
		recN->SetName(SName);
		recN->procInfo->flags = SFlags;
...
// ---------------------------------------------------------------------------
void __fastcall TFEditFunctionDlg_11011981::bRemoveAllClick(TObject *Sender) {
	PInfoRec recN = GetInfoRec(Adr);
	if (recN->procInfo->locals) {
