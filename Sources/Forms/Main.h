...
enum NameVersion {
	nvPrimary, nvAfterScan, nvByUser
};
// XRef Type
...
enum MethodKind {
	MkProcedure, MkFunction, MkConstructor, MkDestructor, MkClassProcedure, MkClassFunction
};
...
enum IntfFlag {
	IfHasGuid, IfDispInterface, IfDispatch
};
...
typedef struct {
	...
	char source;
	// 0 - not defined; 'L' - local var; 'A' - argument; 'M' - memory; 'I' - immediate
	DWORD value;
	String type;
} RINFO, *PRINFO;
// ---------------------------------------------------------------------------
...
__published: // IDE-managed Components
	...
	void __fastcall lbUnitsDrawItem(TWinControl *Control, int Index, TRect &Rect, TOwnerDrawState State);
	void __fastcall FormKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
	void __fastcall lbCodeKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
	...
	void __fastcall lbFormsKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
	void __fastcall tvClassesShortKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
	void __fastcall lbUnitItemsKeyDown(TObject *Sender, WORD &Key, TShiftState Shift);
...
public: // User declarations
	...
	BEGIN_MESSAGE_MAP VCL_MESSAGE_HANDLER(WM_DROPFILES, TWMDropFiles, wm_dropFiles);
