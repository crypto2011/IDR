...
void __fastcall TFTypeInfo_11011981::ShowKbInfo(MTypeInfo* tInfo) {
	...
	if (tInfo->Decl != "")
		memDescription->Lines->Add(tInfo->Decl);
	if (tInfo->FieldsNum) {
...
String __fastcall TFTypeInfo_11011981::GetRTTI(DWORD adr) {
	...
	case ikEnumeration:
		...
		if (SameText(RTTIName, "ByteBool") || SameText(RTTIName, "WordBool") || SameText(RTTIName, "LongBool")) {
			minValue = 0;
			maxValue = 1;
		}
		...
		break;
	case ikFloat:
		if (SameText(RTTIName, "Single") || SameText(RTTIName, "Double") || SameText(RTTIName, "Extended") || SameText(RTTIName, "Comp") || SameText(RTTIName, "Currency"))
			result = "float";
		else {
			floatType = Code[pos];
			pos++;
 ...
 vmtofs = setProc & 0x0000FFFF;
				...
				if (procSig) {
					...
					if (flags != 0xFF) {
						...
						if (paramCount > 0)
							proto += ")";
						if (resultTypeAdr)
							proto += ":" + GetTypeName(resultTypeAdr);
					}
				}
			}
		}
				...
				for (m = 0; m < methCnt; m++) {
					...
					if (flags != 0xFF) {
						...
						if (paramCount > 0)
							result += ")";
						if (resultTypeAdr)
							result += ":" + GetTypeName(resultTypeAdr);
	...
	case ikInterface:
		...
		break;
	case ikInt64:
		minInt64Value = *((__int64*)(Code + pos));
		pos += sizeof(__int64);
	...
	case ikClassRef: // 0x13
		typeAdr = *((DWORD*)(Code + pos));
		if (typeAdr)
			result = "class of " + GetTypeName(typeAdr);
		break;
	case ikPointer: // 0x14
		typeAdr = *((DWORD*)(Code + pos));
		if (typeAdr)
...
// ---------------------------------------------------------------------
void __fastcall TFTypeInfo_11011981::FormKeyDown(TObject *Sender, WORD &Key, TShiftState Shift) {
	if (Key == VK_ESCAPE)
