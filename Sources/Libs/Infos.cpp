...
void __fastcall InfoRec::SetName(String AValue) {
	CrtSection->Enter();
	name = AValue;
	if (ExtractClassName(name) != "" && (kind >= ikRefine && kind <= ikFunc) && procInfo)
		procInfo->flags |= PF_METHOD;
	CrtSection->Leave();
}
...
void __fastcall InfoRec::Load(TStream* ins, char* buf) {
	...
	if (kind == ikResString) {
		...
		ins->Read(&num, sizeof(num));
		if (num)
			vmtInfo->interfaces = new TStringList;
		for (m = 0; m < num; m++) {
			ins->Read(&len, sizeof(len));
			ins->Read(buf, len);
...
String __fastcall InfoRec::MakePrototype(int adr, bool showKind, bool showTail, bool multiline, bool fullName, bool allArgs) {
	...
	if (num) {
		...
		for (n = firstArg; n < argsNum; n++) {
			...
			else if (argInfo->Tag == 0x23)
				result += "const "; // Add by ZGL
	...
	if (showTail) {
		...
		if (procInfo->flags & PF_ARGSIZEG)
			result += "+";
		if (procInfo->flags & PF_ARGSIZEL)
			result += "-";
	}
	return result;
}
...
String __fastcall InfoRec::MakeDelphiPrototype(int Adr, PMethodRec recM) {
	...
	if (SameText(name, "@AbstractError"))
		abstract = true;
	...
	if (num) {
		...
		if (num) {
			...
			for (n = firstArg; n < argsNum; n++) {
				...
				argInfo = (PARGINFO)procInfo->args->Items[n];
				if (argInfo->Tag == 0x22)
					len += sprintf(StringBuf + len, "var ");

				else if (argInfo->Tag == 0x23)
					len += sprintf(StringBuf + len, "const "); // Add by ZGL
...
String __fastcall InfoRec::MakeMultilinePrototype(int Adr, int* ArgsBytes, String MethodType) {
	...
	for (n = firstArg; n < argsNum; n++) {
		...
		// size
		if (argInfo->Size > 4)
			result += ":" + String(argInfo->Size);

		if (argInfo->Ndx > 2)
			argsBytes += argInfo->Size;
	}
	if (num > 0)
		result += "\r\n)";
...
bool __fastcall InfoRec::MakeArgsManually() {
	...
	// @Write2Ext
	if (SameText(name, "@Write2Ext") || SameText(name, "@Str2Ext")) {
		kind = ikProc;
		procInfo->AddArg(0x22, 0, 4, "T", "TTextRec");
