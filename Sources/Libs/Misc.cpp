...
void __fastcall MakeGvar(PInfoRec recN, DWORD adr, DWORD xrefAdr) {
	if (!recN->HasName())
		recN->SetName(MakeGvarName(adr));
	if (xrefAdr)
		recN->AddXref('C', xrefAdr, 0);
}
...
int __fastcall GetNearestArgA(int fromPos) {
	...
	for (curPos = fromPos - 1; ; curPos--) {
		if (IsFlagSet(cfInstruction, curPos)) {
			if (IsFlagSet(cfProcStart, curPos))
				break;
			if (IsFlagSet(cfSetA, curPos))
				return curPos;
		}
	}
	return -1;
...
int __fastcall GetNearestUpPrefixFs(int fromPos) {
	...
	for (_pos = fromPos - 1; _pos >= 0; _pos--) {
		if (IsFlagSet(cfInstruction, _pos)) {
			...
			if (_disInfo.SegPrefix == 4)
				return _pos;
		}
		if (IsFlagSet(cfProcStart, _pos))
			break;
	}
	return -1;
}
...
int __fastcall GetNearestUpInstruction1(int fromPos, int toPos, char* Instruction) {
	...
	for (pos = fromPos - 1; pos >= toPos; pos--) {
		if (IsFlagSet(cfInstruction, pos)) {
			...
			if (len && !memicmp(DisInfo.Mnem, Instruction, len))
				return pos;
		}
		if (IsFlagSet(cfProcStart, pos))
			break;
...
int __fastcall GetNearestUpInstruction2(int fromPos, int toPos, char* Instruction1, char* Instruction2) {
	...
	for (pos = fromPos - 1; pos >= toPos; pos--) {
		if (IsFlagSet(cfInstruction, pos)) {
			Disasm.Disassemble(Pos2Adr(pos), &DisInfo, 0);
			if ((len1 && !memicmp(DisInfo.Mnem, Instruction1, len1)) || (len2 && !memicmp(DisInfo.Mnem, Instruction2, len2)))
				return pos;
		}
		if (IsFlagSet(cfProcStart, pos))
			break;
...
int __fastcall GetNearestDownInstruction(int fromPos, char* Instruction) {
	...
	while (1) {
		...
		if (len && !memcmp(DisInfo.Mnem, Instruction, len))
			return curPos;
		if (DisInfo.Ret)
			break;
		curPos += instrLen;
...
int __fastcall GetProcSize(DWORD fromAdr) {
	...
	if (recN && recN->procInfo)
		_size = recN->procInfo->procSize;
	if (!_size)
		_size = FMain_11011981->EstimateProcSize(fromAdr);
	return _size;
	/*
	 int     pos, size = 0;
...
String __fastcall GetAsmRegisterName(int Idx) {
	assert(Idx >= 0 && Idx < 32);
	if (Idx >= 31)
		return "st(" + String(Idx - 31) + ")";
	if (Idx == 30)
		return "st";
	if (Idx >= 24)
		return String(SegRegTab[Idx - 24]);
	if (Idx >= 16)
		return String(Reg32Tab[Idx - 16]);
	if (Idx >= 8)
		return String(Reg16Tab[Idx - 8]);
...
String __fastcall GetDecompilerRegisterName(int Idx) {
	assert(Idx >= 0 && Idx < 32);
	if (Idx >= 16)
		return UpperCase(Reg32Tab[Idx - 16]);
	if (Idx >= 8)
		return UpperCase(Reg32Tab[Idx - 8]);
...
bool __fastcall IsInheritsByClassName(const String& Name1, const String& Name2) {
	...
	while (adr) {
		if (SameText(GetClsName(adr), Name2))
			return true;
		adr = GetParentAdr(adr);
	}
	return false;
...
bool __fastcall IsInheritsByProcName(const String& Name1, const String& Name2) {
	return (IsInheritsByClassName(ExtractClassName(Name1), ExtractClassName(Name2)) && SameText(ExtractProcName(Name1), ExtractProcName(Name2)));
}
...
String TypeKinds[TypeKindsNum] = {
	"Unknown", "Integer", "Char", "Enumeration", "Float", "ShortString", "Set", "Class", "Method", "WChar", "AnsiString", "WideString", "Variant", "Array", "Record", "Interface", "Int64", "DynArray",
	"UString", "ClassRef", "Pointer", "Procedure"};

// ---------------------------------------------------------------------------
String __fastcall TypeKind2Name(BYTE kind) {
	if (kind < TypeKindsNum)
		return TypeKinds[kind];
	else
		return "";
}
...
String __fastcall GetTypeDeref(String ATypeName) {
	...
	if (ATypeName[1] == '^')
		return ATypeName.SubString(2, ATypeName.Length());
...
int __fastcall GetPackedTypeSize(String AName) {
	int _size;
	if (SameText(AName, "Boolean") || SameText(AName, "ShortInt") || SameText(AName, "Byte") || SameText(AName, "Char")) {
		return 1;
	}
	if (SameText(AName, "SmallInt") || SameText(AName, "Word")) {
		return 2;
	}
	if (SameText(AName, "Dword") || SameText(AName, "Integer") || SameText(AName, "LongInt") || SameText(AName, "LongWord") || SameText(AName, "Cardinal") || SameText(AName, "Single")) {
		return 4;
	}
	if (SameText(AName, "Real48")) {
		return 6;
	}
	if (SameText(AName, "Real") || SameText(AName, "Double") || SameText(AName, "Comp") || SameText(AName, "Currency") || SameText(AName, "Int64")) {
		return 8;
	}
...
String __fastcall GetImmString(String TypeName, int Val) {
	...
	if (!Val && (_kind == ikString || _kind == ikLString || _kind == ikWString || _kind == ikUString))
		return "''";
	if (!Val && (_kind == ikClass || _kind == ikVMT))
		return "Nil";
	...
	if (_kind == ikChar)
		return Format("'%s'", ARRAYOFCONST(((Char)Val)));
	return _default;
}
...
String __fastcall GetEnumerationString(String TypeName, Variant Val) {
	...
	if (Val.Type() == varString)
		return VarToStr(Val);

	_val = Val;

	if (SameText(TypeName, "Boolean") || SameText(TypeName, "ByteBool") || SameText(TypeName, "WordBool") || SameText(TypeName, "LongBool")) {
		if (_val)
			return "True";
		else
			return "False";
...
int __fastcall SortUnitsByOrd(void *item1, void* item2) {
	PUnitRec recU1 = (PUnitRec)item1;
	PUnitRec recU2 = (PUnitRec)item2;
	if (recU1->iniOrder > recU2->iniOrder)
		return 1;
	if (recU1->iniOrder < recU2->iniOrder)
		return -1;
	return 0;
...
bool __fastcall IsBplByExport(const char* bpl) {
	...
	if (hLib) {
		...
		if (pHeader && pHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress && pHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size) {
			pExport = (PIMAGE_EXPORT_DIRECTORY)(DWORD_PTR)(pHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress + imageBase);
			...
			for (int i = 0; i < pExport->NumberOfFunctions; i++) {
				...
				if (index != -1) {
					...
					// lets catch it!
					if (!haveInitializeFunc)
						haveInitializeFunc = (curFunc == "Initialize");
					if (!haveFinalizeFunc)
						haveFinalizeFunc = (curFunc == "Finalize");
					if (!haveGetPackageInfoTableFunc)
						haveGetPackageInfoTableFunc = (curFunc == "@GetPackageInfoTable");
				}
				if (haveInitializeFunc && haveFinalizeFunc && haveGetPackageInfoTableFunc)
					break;
			}
...
int __fastcall IsInitStackViaLoop(DWORD fromAdr, DWORD toAdr) {
	...
	while (curAdr <= toAdr) {
		...
		if (dd == 'dda' && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == 20 && _disInfo.OpType[1] == otIMM) {
			if ((int)_disInfo.Immediate < 0)
				stackSize -= (int)_disInfo.Immediate;
			curAdr += instrLen;
			continue;
		}
		// sub esp, ...
...
// ---------------------------------------------------------------------------
int IdxToIdx32Tab[24] = {16, 17, 18, 19, 16, 17, 18, 19, 16, 17, 18, 19, 20, 21, 22, 23, 16, 17, 18, 19, 20, 21, 22, 23};
...
bool __fastcall IsConnected(DWORD fromAdr, DWORD toAdr) {
	...
	for (n = 0; n < 32; n++) {
		_instrLen = Disasm.Disassemble(Code + _pos, (__int64)_adr, &_disInfo, 0);
		if (_disInfo.Conditional && _disInfo.Immediate == toAdr)
			return true;
		_pos += _instrLen;
		_adr += _instrLen;
	}
...
bool __fastcall IsExit(DWORD fromAdr) {
	...
	if (!IsValidCodeAdr(fromAdr))
		return 0;
	_pos = Adr2Pos(fromAdr);
	_adr = fromAdr;

	while (1) {
		if (!IsFlagSet(cfFinally | cfExcept, _pos))
			break;
		_pos += 8;
		_adr += 8;
		_instrLen = Disasm.Disassemble(Code + _pos, (__int64)_adr, &_disInfo, 0);
	...
	while (1) {
		...
		if (_op == OP_MOV && _disInfo.OpType[0] == otREG && (IsSameRegister(_disInfo.OpRegIdx[0], 16) || IsSameRegister(_disInfo.OpRegIdx[0], 20))) {
			_pos += _instrLen;
			_adr += _instrLen;
			continue;
		}
...
bool __fastcall IsXorMayBeSkipped(DWORD fromAdr) {
	...
	if (_dd == 'rox' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx[1]) {
		...
		_dd = *((DWORD*)_disInfo.Mnem);
		if (_dd == 'vom' && _disInfo.OpType[0] == otREG && IsSameRegister(_disInfo.OpRegIdx[0], _regIdx))
			return true;
	}
	return false;
}
...
int __fastcall IsAbs(DWORD fromAdr) {
	...
	_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
	_dd = *((DWORD*)_disInfo.Mnem);
	if (_dd == 'rox' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == 16 && _disInfo.OpRegIdx[1] == 18) {
		_curPos += _instrLen;
		_curAdr += _instrLen;
		_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
		_dd = *((DWORD*)_disInfo.Mnem);
		if (_dd == 'bus' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == 16 && _disInfo.OpRegIdx[1] == 18) {
...
int _fastcall IsIntOver(DWORD fromAdr) {
	...
	if (_disInfo.Branch && _disInfo.Conditional) {
		...
		if (_disInfo.Call) {
			if (IsValidCodeAdr(_disInfo.Immediate)) {
				_recN = GetInfoRec(_disInfo.Immediate);
				if (_recN && _recN->SameName("@IntOver"))
					return (_curAdr + _instrLen) - fromAdr;
			}
		}
	}
...
int __fastcall IsInlineLengthTest(DWORD fromAdr) {
	...
	_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
	_dd = *((DWORD*)_disInfo.Mnem);
	if (_dd == 'tset' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx[1]) {
		_regIdx = _disInfo.OpRegIdx[0];
		_curPos += _instrLen;
		_curAdr += _instrLen;
		_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
		...
		if (_dd == 'zj' || _dd == 'ej') {
			...
			_dd = *((DWORD*)_disInfo.Mnem);
			// mov reg, [reg-4]
			if (_dd == 'vom' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _regIdx && _disInfo.IndxReg == -1 && _disInfo.Offset == -4) {
				if (_adr == _curAdr + _instrLen)
					return (_curAdr + _instrLen) - fromAdr;
			}
			// sub reg, 4
			if (_dd == 'bus' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.Immediate == 4) {
				_curPos += _instrLen;
				_curAdr += _instrLen;
				_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
				_dd = *((DWORD*)_disInfo.Mnem);
				// mov reg, [reg]
				if (_dd == 'vom' && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _regIdx && _disInfo.IndxReg == -1 && _disInfo.Offset == 0) {
					if (_adr == _curAdr + _instrLen)
						return (_curAdr + _instrLen) - fromAdr;
				}
			}
...
int __fastcall IsInlineLengthCmp(DWORD fromAdr) {
	...
	_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
	_op = Disasm.GetOp(_disInfo.Mnem);
	if (_op == OP_CMP && _disInfo.OpType[0] == otMEM && _disInfo.OpType[1] == otIMM && _disInfo.Immediate == 0) {
		...
		if (_dd == 'zj' || _dd == 'ej') {
			...
			_op = Disasm.GetOp(_disInfo.Mnem);
			// mov reg, [lvar]
			if (_op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _baseReg && _disInfo.Offset == _offset) {
				_regIdx = _disInfo.OpRegIdx[0];
				_curPos += _instrLen;
				_curAdr += _instrLen;
				_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
				_op = Disasm.GetOp(_disInfo.Mnem);
				// sub reg, 4
				if (_op == OP_SUB && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.Immediate == 4) {
					_curPos += _instrLen;
					_curAdr += _instrLen;
					_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
					_op = Disasm.GetOp(_disInfo.Mnem);
					// mov reg, [reg]
					if (_op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == _regIdx && _disInfo.IndxReg == -1 && _disInfo.Offset == 0) {
						_curPos += _instrLen;
						_curAdr += _instrLen;
						_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
						_op = Disasm.GetOp(_disInfo.Mnem);
						// mov [lvar], reg
						if (_op == OP_MOV && _disInfo.OpType[0] == otMEM && _disInfo.OpType[1] == otREG && _disInfo.BaseReg == _baseReg && _disInfo.Offset == _offset) {
							if (_adr == _curAdr + _instrLen)
								return (_curAdr + _instrLen) - fromAdr;
						}
					}
...
int __fastcall IsInlineDiv(DWORD fromAdr, int* div) {
	...
	_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
	_op = Disasm.GetOp(_disInfo.Mnem);
	if (_op == OP_TEST && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx[1]) {
		_regIdx = _disInfo.OpRegIdx[0];
		_curPos += _instrLen;
		_curAdr += _instrLen;
		_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
		...
		if (_dd == 'snj') {
			...
			_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
			_op = Disasm.GetOp(_disInfo.Mnem);
			if (_op == OP_ADD && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _regIdx && _disInfo.OpType[1] == otIMM) {
				_imm = _disInfo.Immediate + 1;
				_curPos += _instrLen;
				_curAdr += _instrLen;
				_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
				_op = Disasm.GetOp(_disInfo.Mnem);
				if (_op == OP_SAR && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _regIdx && _disInfo.OpType[1] == otIMM) {
...
int __fastcall IsInlineMod(DWORD fromAdr, int* mod) {
	...
	_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
	_op = Disasm.GetOp(_disInfo.Mnem);
	if (_op == OP_AND && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && (_disInfo.Immediate & 0x80000000) != 0) {
		_regIdx = _disInfo.OpRegIdx[0];
		...
		if (_dd == 'snj') {
			...
			_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
			_op = Disasm.GetOp(_disInfo.Mnem);
			if (_op == OP_DEC && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _regIdx) {
				_curPos += _instrLen;
				_curAdr += _instrLen;
				_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
				_op = Disasm.GetOp(_disInfo.Mnem);
				if (_op == OP_OR && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.OpRegIdx[0] == _regIdx && _disInfo.Immediate + _imm == 0xFFFFFFFF) {
					_curPos += _instrLen;
					_curAdr += _instrLen;
					_instrLen = Disasm.Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
					_op = Disasm.GetOp(_disInfo.Mnem);
					if (_op == OP_INC && _disInfo.OpType[0] == otREG && _disInfo.OpRegIdx[0] == _regIdx) {
						if (_adr == _curAdr + _instrLen) {
							*mod = _imm + 1;
							return (_curAdr + _instrLen) - fromAdr;
						}
...
bool __fastcall IsDefaultName(String AName) {
	for (int Idx = 0; Idx < 8; Idx++) {
		if (SameText(AName, String(Reg32Tab[Idx])))
			return true;
	}
	if (SameText(AName.SubString(1, 5), "lvar_"))
		return true;
	if (SameText(AName.SubString(1, 5), "gvar_"))
		return true;
	return false;
}
...
int __fastcall FloatNameToFloatType(String AName) {
	if (SameText(AName, "Single"))
		return FT_SINGLE;
	if (SameText(AName, "Double"))
		return FT_DOUBLE;
	if (SameText(AName, "Extended"))
		return FT_EXTENDED;
	if (SameText(AName, "Real"))
		return FT_REAL;
	if (SameText(AName, "Comp"))
		return FT_COMP;
...
bool __fastcall MatchCode(BYTE* code, MProcInfo* pInfo) {
	...
	// jmp XXXXXXXX
	if (_dumpSz == 5 && _dump[0] == 0xE9 && _relocs[1] == 0xFF)
		return false;
	// call XXXXXXXX ret
	if (_dumpSz == 6 && _dump[0] == 0xE8 && _relocs[1] == 0xFF && _dump[5] == 0xC3)
		return false;
...
int __fastcall IsTryBegin(DWORD fromAdr, DWORD *endAdr) {
	...
	for (n = 1; n <= 5; n++) {
		...
		_op = Disasm.GetOp(_disInfo.Mnem);
		if (n == 1) {
			if (!(_op == OP_XOR && _disInfo.OpType[0] == _disInfo.OpType[1] && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx[1]))
				break;
		}
		else if (n == 2) {
			if (!(_op == OP_PUSH && _disInfo.OpRegIdx[0] == 21))
				break;
		}
		else if (n == 3) {
			if (!(_op == OP_PUSH && _disInfo.OpType[0] == otIMM))
				break;
			_endAdr = _disInfo.Immediate;
		}
		else if (n == 4) {
			if (!(_op == OP_PUSH && _disInfo.OpType[0] == otMEM && _disInfo.SegPrefix == 4 && _disInfo.BaseReg != -1))
				break;
		}
		else if (n == 5) {
			if (!(_op == OP_MOV && _disInfo.OpType[0] == otMEM && _disInfo.SegPrefix == 4 && _disInfo.BaseReg != -1))
				break;
			*endAdr = _endAdr;
			return _curAdr + _instrLen - fromAdr;
		}
...
int __fastcall IsTryBegin0(DWORD fromAdr, DWORD *endAdr) {
	...
	for (n = 1; n <= 4; n++) {
		...
		if (n == 1) {
			...
			if (!(_op == OP_PUSH && _disInfo.OpType[0] == otMEM && _disInfo.SegPrefix == 4 && _disInfo.BaseReg == -1 && _disInfo.Offset == 0))
				break;
		}
		else if (n == 4) {
			if (!(_op == OP_MOV && _disInfo.OpType[0] == otMEM && _disInfo.SegPrefix == 4 && _disInfo.BaseReg == -1 && _disInfo.Offset == 0))
				break;
			*endAdr = _endAdr;
			return _curAdr + _instrLen - fromAdr;
		}
...
int __fastcall IsTryEndPush(DWORD fromAdr, DWORD* endAdr) {
	...
	for (n = 1; n <= 6; n++) {
		...
		if (n == 1) {
			...
			if (!(_op == OP_POP && _disInfo.OpType[0] == otREG))
				break;
		}
		else if (n == 5) {
			if (!(_op == OP_MOV && _disInfo.OpType[0] == otMEM && _disInfo.SegPrefix == 4 && _disInfo.BaseReg != -1))
...
int __fastcall IsTryEndJump(DWORD fromAdr, DWORD* endAdr) {
	...
	for (n = 1; n <= 6; n++) {
		...
		if (n == 1) {
			...
			if (!(_op == OP_JMP && _disInfo.OpType[0] == otIMM))
				break;
			*endAdr = _disInfo.Immediate;
...
int __fastcall ProcessInt64Equality(DWORD fromAdr, DWORD* maxAdr) {
	...
	for (n = 1; n <= 1024; n++) {
		...
		{
			if (!(_disInfo.Branch && _disInfo.Conditional && _b == 'F'))
				break;
			_skipAdr2 = _curAdr;
		...
		{
			if (!(_disInfo.Branch && _disInfo.Conditional && _b == 'F' && _disInfo.Immediate == _adr1))
				break;
			*maxAdr = _maxAdr;
...
int __fastcall ProcessInt64NotEquality(DWORD fromAdr, DWORD* maxAdr) {
	...
	for (n = 1; n <= 1024; n++) {
		...
		{
			if (!(_disInfo.Branch && _disInfo.Conditional && _b == 'E' && _curAdr + _instrLen == _adr1))
				break;
...
int __fastcall ProcessInt64Comparison(DWORD fromAdr, DWORD* maxAdr) {
	...
	for (n = 1; n <= 1024; n++) {
		...
		else if (n == 5) // jmp @@
		{
			...
			////@1:jxx @@
		{
			if (!(_disInfo.Branch && _disInfo.Conditional && _curAdr == _adr1))
				break;
			_adr = _disInfo.Immediate; // @@
			if (_adr > _maxAdr)
				_maxAdr = _adr;
...
int __fastcall ProcessInt64ComparisonViaStack1(DWORD fromAdr, DWORD* maxAdr) {
	...
	for (n = 1; n <= 1024; n++) {
		...
		if (n == 1) // push reg
		{
			if (!(_op == OP_PUSH && _disInfo.OpType[0] == otREG))
				break;
		}
		...
		{
			if (!(_op == OP_CMP && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == 20 && _disInfo.Offset == 0))
				break;
		}
		else if (m != -1 && n == m + 3) // @1:pop reg
		{
			if (!(_op == OP_POP && _disInfo.OpType[0] == otREG && _curAdr == _adr1))
				break;
		}
		...
		if (m == -1 && (_disInfo.Ret || _disInfo.Branch))
			return 0;
		_curAdr += _instrLen;
	}
	return 0;
...
int __fastcall IsInt64Shr(DWORD fromAdr) {
	...
	for (n = 1; n <= 2; n++) {
		...
		_op = Disasm.GetOp(_disInfo.Mnem);
		if (n == 1) {
			if (!(_op == OP_SHR && _disInfo.OpNum == 3 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpType[2] == otIMM))
				break;
			_idx = _disInfo.OpRegIdx[1];
			_val = _disInfo.Immediate;
		}
		else if (n == 2) {
			if (!(_op == OP_SHR && _disInfo.OpNum == 2 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM && _disInfo.OpRegIdx[0] == _idx && _disInfo.Immediate == _val))
				break;
			return _curAdr + _instrLen - fromAdr;
		}
		if (_disInfo.Ret)
...
int __fastcall IsInt64Shl(DWORD fromAdr) {
	...
	for (n = 1; n <= 2; n++) {
		...
		_op = Disasm.GetOp(_disInfo.Mnem);
		if (n == 1) {
			if (!(_op == OP_SHL && _disInfo.OpNum == 3 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpType[2] == otIMM))
				break;
			_idx = _disInfo.OpRegIdx[1];
			_val = _disInfo.Immediate;
		}
