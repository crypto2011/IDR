...
int __fastcall TAnalyzeThread::StartProgress(int pbMaxCount, const String& sbText) {
	...
	if (pbSteps * stepSize < pbMaxCount)
		pbSteps++;
...
void __fastcall TAnalyzeThread::FindRTTIs() {
	...
	for (int i = 0; i < CodeSize && !Terminated; i += 4) {
		...
		if (IsValidImageAdr(adr) && adr == Pos2Adr(i) + 4) {
			...
			if (instrLen > 3 && DisInfo.Branch && DisInfo.Offset == adr)
				continue;

			DWORD typeAdr = adr - 4;
			BYTE typeKind = Code[i + 4];
			if (typeKind == ikUnknown || typeKind > ikProcedure)
				continue;

			BYTE len = Code[i + 5];
			if (!IsValidName(len, i + 6))
				continue;
			...
			case ikEnumeration: // 3
				...
				if (baseTypeAdr == typeAdr) {
					if (SameText(TypeName, "ByteBool") || SameText(TypeName, "WordBool") || SameText(TypeName, "LongBool")) {
						minValue = 0;
						maxValue = 1;
					}
			...
			case ikMethod: // 8
				// MethodKind
				methodKind = Code[n];
				n++; // 0 (mkProcedure) or 1 (mkFunction)
				paramCnt = Code[n];
				n++;
...
void __fastcall TAnalyzeThread::FindVMTs2() {
	...
	for (i = 0; i < CodeSize && !Terminated; i += 4) {
		if ((i & stepMask) == 0)
			UpdateProgress();
		if (IsFlagSet(cfCode | cfData, i))
			continue;

		DWORD selfPtr = *((DWORD*)(Code + i));
		if (selfPtr && !IsValidImageAdr(selfPtr))
			continue;

		DWORD initTableAdr = *((DWORD*)(Code + i + 4));
		if (initTableAdr) {
			if (!IsValidImageAdr(initTableAdr))
				continue;
			if (*(Code + pos) != 0xE)
				continue;
		...
		if (fieldTableAdr) {
			if (!IsValidImageAdr(fieldTableAdr))
				continue;
		...
		if (methodTableAdr) {
			if (!IsValidImageAdr(methodTableAdr))
				continue;
		...
		if (dynamicTableAdr) {
			if (!IsValidImageAdr(dynamicTableAdr))
				continue;
		...
		if (!classNameAdr || !IsValidImageAdr(classNameAdr))
			continue;
		...
		if (parentAdr && !IsValidImageAdr(parentAdr))
			continue;

		DWORD defaultHandlerAdr = *((DWORD*)(Code + i + 0x24));
		if (defaultHandlerAdr && !IsValidImageAdr(defaultHandlerAdr))
			continue;

		DWORD newInstanceAdr = *((DWORD*)(Code + i + 0x28));
		if (newInstanceAdr && !IsValidImageAdr(newInstanceAdr))
			continue;

		DWORD freeInstanceAdr = *((DWORD*)(Code + i + 0x2C));
		if (freeInstanceAdr && !IsValidImageAdr(freeInstanceAdr))
			continue;

		DWORD destroyAdr = *((DWORD*)(Code + i + 0x30));
		if (destroyAdr && !IsValidImageAdr(destroyAdr))
			continue;

		DWORD classVMT = Pos2Adr(i) - VmtSelfPtr;
		if (Adr2Pos(classVMT) < 0)
			continue;

		int StopAt = GetStopAt(classVMT);
		if (i + StopAt - classVMT - VmtSelfPtr >= CodeSize)
			continue;
		...
		if (recU) {
			if (typeInfoAdr) // extract unit name
			{
				...
				if (!IsValidName(len, _ap))
					continue;
				String unitName = String((char*)(Code + _ap), len).Trim();
				_ap += len;
				FMain_11011981->SetUnitName(recU, unitName);
...
void __fastcall TAnalyzeThread::FindVMTs() {
	...
	for (int i = 0; i < TotalSize && !Terminated; i += 4) {
		...
		if (IsValidImageAdr(adr) && Pos2Adr(i) == adr + VmtSelfPtr) {
			...
			if (intfTableAdr) {
				if (!IsValidImageAdr(intfTableAdr))
					continue;
			...
			if (autoTableAdr) {
				if (!IsValidImageAdr(autoTableAdr))
					continue;
			...
			if (intfTableAdr) {
				...
				for (int m = 0; m < EntryCount; m++) {
					...
					if (IsValidImageAdr(vTableAdr))
						SetFlag(cfData | cfVTable, Adr2Pos(vTableAdr));
					// IOffset
					pos += 4;
					bytes += 4;
					if (DelphiVersion > 3) {
...
void __fastcall TAnalyzeThread::FindPrototypes() {
	...
	for (n = 0; n < ResInfo->FormList->Count && !Terminated; n++) {
		...
		for (m = 0; m < ev->Count && !Terminated; m++) {
			...
			if (typeName == "")
				typeName = FindEvent(controlAdr, eInfo->EventName);
			if (typeName != "") {
...
StdUnitInfo StdUnits[] = {
	{false, "Types", 0.0, -1}, {false, "Multimon", 0.0, -1}, {false, "VarUtils", 0.0, -1}, {false, "StrUtils", 0.0, -1}, {false, "Registry", 0.0, -1}, {false, "IniFiles", 0.0, -1},
	{false, "Clipbrd", 0.0, -1}, {false, 0, 0.0, -1}};
...
void __fastcall TAnalyzeThread::ScanCode() {
				...
				if (Idx != -1) {
					...
					if (!KnowledgeBase.IsUsedProc(Idx)) {
						if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo)) {
							...
							if (matched)
								mainForm->StrapProc(Adr2Pos(iniAdr), Idx, pInfo, true, pInfo->DumpSz);
						}
					}
				}
			}
				...
				if (Idx != -1) {
					...
					if (!KnowledgeBase.IsUsedProc(Idx)) {
						if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo)) {
							...
							if (matched)
								mainForm->StrapProc(Adr2Pos(finAdr), Idx, pInfo, true, pInfo->DumpSz);
						}
					}
				}
			}
	...
	for (r = 0; !Terminated; r++) {
		...
		if (!KnowledgeBase.GetProcIdxs(moduleID, &FirstProcIdx, &LastProcIdx))
			continue;
			...
			if (!recU->names->Count) {
				...
				for (m = fromPos; m < toPos && !Terminated; m++) {
					if (IsFlagSet(cfProcStart, m)) {
						if (r != 5 && Pos2Adr(m) == recU->iniadr)
							continue;
						if (r != 5 && Pos2Adr(m) == recU->finadr)
							continue;
	...
	for (n = 0; n < UnitsNum && !Terminated; n++) {
		...
		for (u = 0; u < recU->names->Count && !Terminated; u++) {
			...
			if (!KnowledgeBase.GetProcIdxs(moduleID, &FirstProcIdx, &LastProcIdx, &DumpSize))
				continue;
			...
			for (m = fromPos, i = 0; m < toPos && !Terminated; m++, i++) {
				...
				if (IsFlagSet(cfProcStart, m) || !Flags[m]) {
					if (Pos2Adr(m) == recU->iniadr && recU->trivialIni)
						continue;
					if (Pos2Adr(m) == recU->finadr && recU->trivialFin)
						continue;
					recN = GetInfoRec(Pos2Adr(m));
					if (recN && recN->HasName())
						continue;
	...
	for (r = 0; !Terminated; r++) {
		...
		for (n = 0; n < UnitsNum && !Terminated; n++) {
			...
			if (!recU->names->Count) {
				...
				for (m = fromPos; m < toPos && !Terminated; m++) {
					if (IsFlagSet(cfProcStart, m)) {
						if (Pos2Adr(m) == recU->iniadr)
							continue;
						if (Pos2Adr(m) == recU->finadr)
							continue;
					...
					for (n = Adr2Pos(recU->fromAdr); (n < Adr2Pos(recU->toAdr)) && !Terminated; n += 4) {
						...
						if (IsValidImageAdr(adr) && adr == HInstanceVarAdr && resid < 0x10000) {
							...
							if (!recN)
								...
								// may be ikData
								if (!recN->rsInfo)
									recN->rsInfo = new InfoResStringInfo;
							}
	...
	for (int m = 0; m < OwnTypeList->Count && !Terminated; m++) {
		...
		if (recT->kind == ikClass) {
			...
			for (int i = 0; i < propCount && !Terminated; i++) {
				...
				len = Code[n];
				n++;
				String fieldName = String((char*)(Code + n), len);
				n += len;
				...
				if (recN1 && fieldOfs != -1)
					recN1->vmtInfo->AddField(0, 0, FIELD_PUBLIC, fieldOfs, -1, fieldName, fieldType);
			}
		}
	}
}
...
void __fastcall TAnalyzeThread::FindStrings() {
	...
	if (DelphiVersion >= 2009) {
		...
		for (i = 0; i < CodeSize && !Terminated; i += 4) {
			if ((i & stepMask) == 0)
				UpdateProgress();
			if (IsFlagSet(cfData, i))
				continue;

			codePage = *((WORD*)(Code + i));
			elemSize = *((WORD*)(Code + i + 2));
			if (!elemSize || elemSize > 4)
				continue;
			...
			if (len <= 0 || len > 10000)
				continue;
			if (i + 12 + (len + 1) * elemSize >= CodeSize)
				continue;
			if (!Infos[i + 12]) {
