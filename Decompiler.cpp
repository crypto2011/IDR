...
{
    Env->FStack[(_TOP_ + idx) & 7].Flags = val->Flags;
    Env->FStack[(_TOP_ + idx) & 7].Precedence = val->Precedence;
    Env->FStack[(_TOP_ + idx) & 7].Size = val->Size;
    Env->FStack[(_TOP_ + idx) & 7].Offset = val->Offset;
    Env->FStack[(_TOP_ + idx) & 7].IntValue = val->IntValue;
    Env->FStack[(_TOP_ + idx) & 7].Value = val->Value;
    Env->FStack[(_TOP_ + idx) & 7].Type = val->Type;
    Env->FStack[(_TOP_ + idx) & 7].Name = val->Name;
}
//---------------------------------------------------------------------------
...
_curAdr = _curAdr;
                    ...
                    if (_loopInfo->Kind == 'F')
                    {
                        ...
                        if (_loopInfo->forInfo->NoVar)
                        {
                            ...
                            if (_loopInfo->forInfo->CntInfo.IdxType == itLVAR)
                                _line += Env->GetLvarName((int)_varidx);
                        }
                        else
                        {
        ...
        if (_op == OP_MOVS)
        {
            ...
            if (_item1.Flags & IF_STACK_PTR)
            {
                ...
                Env->AddToBody(Env->GetLvarName(_item1.IntValue) + " := " + _item2.Value + ";");
                _curPos += _instrLen; _curAdr += _instrLen;
                continue;
            }
            //lvar->
            if (_item2.Flags & IF_STACK_PTR)
            {
                ...
                Env->AddToBody(_item1.Value + " := " + Env->GetLvarName(_item2.IntValue) + ";");
                _curPos += _instrLen; _curAdr += _instrLen;
                continue;
            }
            if (_item1.Value != "")
...
bool __fastcall TDecompiler::SimulateCall(DWORD curAdr, DWORD callAdr, int instrLen, PMethodRec recM, DWORD AClassAdr)
{
            ...
            if (_callKind == 0 || _callKind == 3 || _callKind == 2 || _callKind == 1)//fastcall, stdcall, pascal, cdecl 
            {
                ...
                for (int n = 0; n < _argsNum; n++)
                {
                    ...
                    if (_item.Flags & IF_STACK_PTR)
                    {
                        ...
                        if (_kind == ikInteger)
                        {
                            _line += Env->GetLvarName(_item.IntValue);
                            Env->Stack[_item.IntValue].Value = Env->GetLvarName(_item.IntValue);
                            continue;
                        }
                    ...
                    if (_kind == ikRecord)
                    {
                        _line += _item.Value;//ExtractClassName(_item.Value);
                        continue;
                    }
                    if (_kind == ikFloat)
                    {
                    ...
                    _retKind == ikArray)
                {
                    ...
                    if (_item.Flags & IF_STACK_PTR)
                    {
                        if (_item.Name != "")
                            ...
                            _line = Env->GetLvarName(_item.IntValue) + " := " + _line;
    ...
    if (DisInfo.BaseReg != -1)
    {
        ...
        if (IsValidImageAdr(_classAdr))
        {
            ...
            //Field
            _fInfo = FMain_11011981->GetField(_item.Type, DisInfo.Offset, &_vmt, &_vmtAdr);
            if (!_fInfo)
            {
                while (!_recM)
                {
...
//---------------------------------------------------------------------------
void __fastcall TDecompiler::SimulatePush(DWORD curAdr)
{
    bool        _vmt;
    int         _offset, _idx;
    DWORD       _vmtAdr;
    ...
    if (DisInfo.OpType[0] == otREG)
    {
        ...
        _value = GetDecompilerRegisterName(_idx);
        if (_item.Value != "") _value = _item.Value + "{" + _value + "}";
        _item.Value = _value;
...
void __fastcall TDecompiler::SimulatePop(DWORD curAdr)
{
    ...
    if (DisInfo.OpType[0] == otREG)
    {
        ...
        if (_item->Flags & IF_ARG)
        {
            ...
            {
                _line = GetDecompilerRegisterName(DisInfo.OpRegIdx[0]) + " := " + _item->Name + ";";
                Env->AddToBody(_line);
            }
        }
            ...
            if (DisInfo.IndxReg == -1)
            {
                ...
                if (DisInfo.BaseReg == 20)
                {
                    if (Op == OP_IMUL)
                    {
                        ...
                        SetRegItem(16, &_item);
                        SetRegItem(18, &_item);
                        _line = "EDX_EAX := EAX * " + Env->GetLvarName(_ESP_ + _offset) + ";//" + _item1.Value;
                        Env->AddToBody(_line);
                        return;
                    }
                }
                ...
                if (DisInfo.BaseReg == 21 && (_itemBase.Flags & IF_STACK_PTR))
                {
                    if (Op == OP_IMUL)
                    {
                        ...
                        if (_item1.Value != "")
                            ...
                            _name = Env->GetLvarName(_itemBase.IntValue + _offset);
                        ...
                        SetRegItem(16, &_item);
                        SetRegItem(18, &_item);
                        _line = "EDX_EAX := EAX * " + Env->GetLvarName(_ESP_ + _offset) + ";//" + _item.Value;
                        Env->AddToBody(_line);
                        return;
                    }
                }
                if (_itemBase.Type[1] == '^')   //Pointer to gvar (from other unit)
                {
                    if (Op == OP_IMUL)
                    {
                        ...
                        InitItem(&_item);
                        _item.Precedence = PRECEDENCE_MULT;
                        _item.Value = GetString(&_item2, PRECEDENCE_MULT) + " * " + GetString(&_itemBase, PRECEDENCE_MULT);
                        _item.Type = "Integer";
                        SetRegItem(16, &_item);
                        SetRegItem(18, &_item);
...
void __fastcall TDecompiler::SimulateInstr2RegImm(DWORD curAdr, BYTE Op)
{
    ...
    PLOCALINFO  _locInfo;
    PFIELDINFO  _fInfo;
    String      _name, _value, _typeName, _line, _comment, _imm, _iname, _fname, _text;
    WideString  _wStr;
    ...
    if (Op == OP_ADD)
    {
        ...
        if (_item1.Type != "")
        {
            ...
            _fInfo = FMain_11011981->GetField(_item1.Type, DisInfo.Immediate, &_vmt, &_vmtAdr);
            if (_fInfo)
            {
                InitItem(&_item);
                _name = GetFieldName(_fInfo);
    ...
    if (Op == OP_OR)
    {
        ...
        {
            _item.Precedence = PRECEDENCE_ADD;
            _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " Or " + _imm;
            _item.Type = "Cardinal";
        }
        SetRegItem(_reg1Idx, &_item);
...
void __fastcall TDecompiler::SimulateInstr2RegReg(DWORD curAdr, BYTE Op)
{
    ...
    if (Op == OP_ADD || Op == OP_SUB || Op == OP_OR || Op == OP_XOR)
    {
        ...
        _comment = GetString(&_item1, PRECEDENCE_ADD) + _op + GetString(&_item2, PRECEDENCE_ADD);
        Env->AddToBody(_line + ";//" + _comment);
    ...
    {
        _line += GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + _op + GetDecompilerRegisterName(_reg2Idx);
        _comment = GetString(&_item1, PRECEDENCE_MULT) + _op + GetString(&_item2, PRECEDENCE_MULT);
        Env->AddToBody(_line + ";//" + _comment);
    ...
    if (Op == OP_DIV || Op == OP_IDIV)
    {
        ...
        _comment = GetString(&_item1, PRECEDENCE_MULT) + " Div " + GetString(&_item2, PRECEDENCE_MULT);
        Env->AddToBody(_line + ";//" + _comment);
        ...
        _item.Type = "Integer";
        SetRegItem(16, &_item);
        _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " Mod " + GetString(&_item2, PRECEDENCE_MULT);;
        SetRegItem(18, &_item);
        return;
    }
    if (Op == OP_XCHG)
...
void __fastcall TDecompiler::SimulateInstr2RegMem(DWORD curAdr, BYTE Op)
{
    ...
    if (_itemSrc.Flags & IF_STACK_PTR)
    {
        ...
        if (Op == OP_MOV)
        {
            ...
            if (_item.Flags & IF_FIELD)
            {
                ...
                if (_name == "")
                    _name = Env->GetLvarName(_itemSrc.IntValue - _foffset);
                InitItem(&_itemDst);
                if (_fname.Pos(":"))
                {
    ...
    if (Op == OP_ADD || Op == OP_SUB || Op == OP_XOR)
    {
        InitItem(&_item);
        _item.Precedence = PRECEDENCE_ADD;
        _item.Value = GetString(&_itemDst, PRECEDENCE_ADD) + _op + GetString(&_itemSrc, PRECEDENCE_ADD);
        _item.Type = _itemSrc.Type;
        SetRegItem(_reg1Idx, &_item);
    ...
    if (Op == OP_MUL || Op == OP_IMUL || Op == OP_AND)
    {
        InitItem(&_item);
        _item.Precedence = PRECEDENCE_MULT;
        _item.Value = GetString(&_itemDst, PRECEDENCE_MULT) + _op + GetString(&_itemSrc, PRECEDENCE_MULT);
        _item.Type = _itemSrc.Type;
        SetRegItem(_reg1Idx, &_item);
        _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + _op + _itemSrc.Value + ";//" + _item.Value;
        Env->AddToBody(_line);
    ...
    if (Op == OP_DIV || Op == OP_IDIV)
    {
        InitItem(&_item);
        _item.Precedence = PRECEDENCE_MULT;
        _item.Value = GetString(&_itemDst, PRECEDENCE_MULT) + " Div " + GetString(&_itemSrc, PRECEDENCE_MULT);
        _item.Type = _itemSrc.Type;
        SetRegItem(16, &_item);

        InitItem(&_item);
        _item.Precedence = PRECEDENCE_MULT;
        _item.Value = GetString(&_itemDst, PRECEDENCE_MULT) + " Mod " + GetString(&_itemSrc, PRECEDENCE_MULT);
...
bool __fastcall TDecompiler::SimulateSysCall(String name, DWORD procAdr, int instrLen)
{
    ...
    if (SameText(name, "@CopyRecord"))
    {
        ...
        if (_item1.Flags & IF_STACK_PTR)
        {
            Env->Stack[_item1.IntValue].Value = Env->GetLvarName(_item1.IntValue);
        ...
        Env->Stack[_item1.IntValue].Type = _recN->GetName();
    ...
    if (SameText(name, "@DynArrayAsg"))
    {
        ...
        if (_item1.Flags & IF_STACK_PTR)
        {
            if (Env->Stack[_item1.IntValue].Value == "") Env->Stack[_item1.IntValue].Value = Env->GetLvarName(_item1.IntValue);
            _item1 = Env->Stack[_item1.IntValue];
        }
        //edx - src
        GetRegItem(18, &_item2);
        if (_item2.Flags & IF_STACK_PTR)
        {
            if (Env->Stack[_item2.IntValue].Value == "") Env->Stack[_item2.IntValue].Value = Env->GetLvarName(_item2.IntValue);
    ...
    if (SameText(name, "@DynArraySetLength"))//stdcall
    {
        ...
        GetRegItem(18, &_item2);
        Env->Stack[_item1.IntValue].Type = GetTypeName(_item2.IntValue);
        //ecx - dims cnt
        ...
        SameText(name, "@FinalizeArray"))
    {
        ...
        //ecx - dims cnt
        GetRegItem(17, &_item3); _cnt = _item3.IntValue;
        Env->Stack[_item1.IntValue].Type = "array [1.." + String(_cnt) + "] of " + GetTypeName(_item2.IntValue);;
        return false;
    }
...
//Add analyze of String type
    ...
    if (SameText(name, "@UStrFromWChar"))
    {
        //eax-Dst
        GetRegItem(16, &_item1);
        if (_item1.Flags & IF_STACK_PTR)
        ...
        SameText(name, "@WStrFromLStr"))
    {
        ...
        {
            _item = Env->Stack[_item1.IntValue];
            _item.Value = Env->GetLvarName(_item1.IntValue);
            _item.Type = "WideString";
...
void __fastcall TDecompiler::SimulateFloatInstruction(DWORD curAdr, int instrLen)
{
    ...
    PInfoRec    _recN;
    PFIELDINFO  _fInfo;
    String      _name, _typeName, _val, _line;
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "st")) != 0)
    {
        ...
        if (DisInfo.OpType[0] == otMEM)
        {
            ...
            if (_itemSrc.Flags & IF_STACK_PTR)
            {
                ...
                _line = Env->GetLvarName(_itemSrc.IntValue) + " := " + _item.Value + ";";
                Env->AddToBody(_line);
                _item.Value = Env->GetLvarName(_itemSrc.IntValue);
            ...
            if (DisInfo.OpType[0] == otMEM)
            {
                ...
                if (_itemSrc.Flags & IF_STACK_PTR)
                {
                    ...
                    if (_item.Value != "")
                        ...
                        CmpInfo.R = Env->GetLvarName(_itemSrc.IntValue);
                    if (_p)
                    {
                        FPop();
                        //fcompp
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "add")) != 0)
    {
            ...
            if (DisInfo.OpType[0] == otMEM)
            {
                ...
                if (_itemSrc.Flags & IF_STACK_PTR)
                {
                    ...
                    if (_item.Value != "")
                        ...
                        _val = Env->GetLvarName(_itemSrc.IntValue);
        ...
        if (DisInfo.OpNum == 2)
        {
            ...
            InitItem(&_item);
            _item.Precedence = PRECEDENCE_ADD;
            _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_ADD) + " + " + GetString(FGet(_reg2Idx), PRECEDENCE_ADD);
            _item.Type = "Extended";
            FSet(_reg1Idx, &_item);
            //faddp
            if (_p) FPop();
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "sub")) != 0)
    {
        ...
        if (DisInfo.OpNum == 1)
        {
            if (DisInfo.OpType[0] == otMEM)
            {
                ...
                _item.Precedence = PRECEDENCE_ADD;
                if (_r)
                    _item.Value = _itemSrc.Value + " - " + GetString(FGet(0), PRECEDENCE_ADD);
                else
                    _item.Value = GetString(FGet(0), PRECEDENCE_ADD) + " - " + _itemSrc.Value;
                _item.Type = "Extended";
                FSet(0, &_item);
        ...
        if (DisInfo.OpNum == 2)
        {
            ...
            _item.Precedence = PRECEDENCE_ADD;
            if (_r)
                _item.Value = GetString(FGet(_reg2Idx), PRECEDENCE_ADD) + " - " + GetString(FGet(_reg1Idx), PRECEDENCE_ADD);
            else
                _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_ADD) + " - " + GetString(FGet(_reg2Idx), PRECEDENCE_ADD);
            _item.Type = "Extended";
            FSet(_reg1Idx, &_item);
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "mul")) != 0)
    {
        ...
        if (DisInfo.OpNum == 2)
        {
            ...
            InitItem(&_item);
            _item.Precedence = PRECEDENCE_MULT;
            _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_MULT) + " * " + GetString(FGet(_reg2Idx), PRECEDENCE_MULT);
            _item.Type = "Extended";
            FSet(_reg1Idx, &_item);
            //fmulp
            if (_p) FPop();
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "div")) != 0)
    {
        ...
        if (DisInfo.OpNum == 1)
        {
            if (DisInfo.OpType[0] == otMEM)
            {
                ...
                _item.Precedence = PRECEDENCE_MULT;
                if (_r)
                    _item.Value = _itemSrc.Value + " / " + GetString(FGet(0), PRECEDENCE_MULT);
                else
                    _item.Value = GetString(FGet(0), PRECEDENCE_MULT) + " / " + _itemSrc.Value;
                _item.Type = "Extended";
                FSet(0, &_item);
        ...
        if (DisInfo.OpNum == 2)
        {
            ...
            _item.Precedence = PRECEDENCE_MULT;
            if (_r)
                _item.Value = GetString(FGet(_reg2Idx), PRECEDENCE_MULT) + " / " + GetString(FGet(_reg1Idx), PRECEDENCE_MULT);
            else
                _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_MULT) + " / " + GetString(FGet(_reg2Idx), PRECEDENCE_MULT);
            _item.Type = "Extended";
            FSet(_reg1Idx, &_item);
...
//---------------------------------------------------------------------------
PFIELDINFO __fastcall TDecompiler::GetArrayFieldOffset(String ATypeName, int AFromOfs, int AScale)
{
    bool        _vmt;
    DWORD       _vmtAdr;
    int         _size, _lIdx, _hIdx;
    int         _classSize = GetClassSize(GetClassAdr(ATypeName));
    int         _offset = AFromOfs, _ofs;
    PFIELDINFO  _fInfo;

    while (1)
    {
        if (_offset >= _classSize) break;
        _fInfo = FMain_11011981->GetField(ATypeName, _offset, &_vmt, &_vmtAdr);
        if (_fInfo && GetTypeKind(_fInfo->Type, &_size) == ikArray && GetArrayIndexes(_fInfo->Type, 1, &_lIdx, &_hIdx))
        {
            _size = GetArrayElementTypeSize(_fInfo->Type);
            _ofs = AFromOfs + AScale * _lIdx;
            if (_ofs >= _fInfo->Offset && _ofs <= _fInfo->Offset + (_hIdx - _lIdx) * _size)
                return _fInfo;
        }
        //Try next offset
        _offset++;
...
void __fastcall TDecompiler::GetMemItem(int CurAdr, PITEM Dst, BYTE Op)
{
    bool        _vmt;
    BYTE        _kind;
    int         _offset, _foffset, _pos, _size, _idx, _idx1, _mod;
    DWORD       _adr, _vmtAdr;
    PInfoRec    _recN, _recN1;
    PFIELDINFO  _fInfo;
    ITEM        _item, _itemBase, _itemIndx;
    String      _fname, _name, _typeName, _iname, _value, _text;
    ...
    if (DisInfo.IndxReg == -1)
    {
        ...
        if (DisInfo.BaseReg == 20)
        {
            ...
            if (_item.Flags & IF_FIELD)
            {
                ...
                _name = Env->GetLvarName(_ESP_ + _offset);
                if (_fname.Pos(":"))
                {
                    Dst->Value = _name + "." + ExtractName(_fname);
                    Dst->Type = ExtractType(_fname);
                }
                else
                {
        ...
        if (_itemBase.Flags & IF_STACK_PTR)
        {
            ...
            {
                Env->Stack[_itemBase.IntValue + _offset].Name = Env->GetLvarName(_itemBase.IntValue + _offset);
                Dst->Flags = IF_STACK_PTR;
                Dst->IntValue = _itemBase.IntValue + _offset;
                Dst->Value = Env->GetLvarName(_itemBase.IntValue + _offset);
                Dst->Name = Dst->Value;
                return;
            }
        }
...
//            return;
        ...
        else if (_kind == ikVMT || _kind == ikClass)
        {
            ...
            if (!_fInfo)
            {
                ...
                {
                    Env->ErrAdr = CurAdr;
                    throw Exception("Field f" + Val2Str0(_offset) + " not found in specified type");
                }
            }
            _value = GetFieldName(_fInfo);
            _typeName = _fInfo->Type;
            //if (Op != OP_LEA)
            //{
                _kind = GetTypeKind(_typeName, &_size);
                //Interface
                ...
                if (_kind == ikRecord)
                {
                    _text = GetRecordFields(_offset - _fInfo->Offset, _typeName);
                    if (_text == "")
                    {
                        _text = ManualInput(CurProcAdr, CurAdr, "Define [name:]type of field " + _typeName + ".f" + Val2Str0(_offset - _fInfo->Offset), "[Name]:Type:");
                        if (_text == "")
                        {
                            Env->ErrAdr = CurAdr;
                            throw Exception("See ya later!");
                ...
                if (_kind == ikArray)
                {
                    _value += "[ofs=" + String(_offset - _fInfo->Offset) + "]";
                }
    ...
    else
    {
        ...
        {
            _fInfo = GetArrayFieldOffset(_typeName, _offset, DisInfo.Scale);
            while (!_fInfo)
            {
                _text = ManualInput(CurProcAdr, CurAdr, "Define actual offset of array field", "Offset (in hex):");
                if (_text == "")
                {
                    Env->ErrAdr = CurAdr;
                    throw Exception("See ya later!");
            ...
            if (_fInfo->Name != "")
                ...
                _value += "f" + Val2Str0(_fInfo->Offset);
            _value += "[" + GetDecompilerRegisterName(DisInfo.IndxReg) + "]";

            Dst->Value = _value;
            Dst->Type = GetArrayElementType(_fInfo->Type);
            return;
        }
...
String __fastcall TDecompiler::GetStringArgument(PITEM item)
{
    ...
    if (item->Flags & IF_STACK_PTR)
    {
        ...
        return Env->GetLvarName(item->IntValue);
    }
